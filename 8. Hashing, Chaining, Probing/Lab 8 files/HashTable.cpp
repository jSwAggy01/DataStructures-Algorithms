...
void HashTable::put(const string &s, int score) {
	int hashedS = computeHash(s);
	list<WordEntry>::iterator nextWord;

	for(nextWord = hashTable[hashedS].begin(); nextWord != hashTable[hashedS].end(); nextWord++){
		if(nextWord->getWord() == s){
			nextWord->addNewAppearance(score);
			return;
		}
	...
	hashTable[hashedS].push_back(newEntry);
}
...
double HashTable::getAverage(const string &s) {
	...
	for(nextWord = hashTable[hashedS].begin(); nextWord != hashTable[hashedS].end(); nextWord++){
		if(nextWord->getWord() == s){
			double avg = nextWord->getAverage();
			return avg;
		}
	}
